...
func (u *UserHandlerImpl) Login(w http.ResponseWriter, r *http.Request) error {
	functionName := "UserHandlerImpl.Login"

	ctx := r.Context()

	payload := new(usersreqdto.LoginRequest)
	...
	result, err := u.userService.Login(ctx, payload)
	if err != nil {
		return err
	}
